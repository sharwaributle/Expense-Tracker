...
def ai_audit():
    ...
    prompt = (
        ...
        f"Give exactly 2 short sentences of proactive financial advice in clear, professional English. "
        f"Use only these numbers. Mention the actual over-budget category and its actual "
        f"spent/limit values if one exists. If none is over budget, confirm that spending is within limits. "
    ...
    if overspent:
        ...
        insight = (
            f"{cat} has exceeded its budget limit: ₹{spent:.0f} spent vs ₹{cap:.0f} limit. "
            f"Consider reducing discretionary expenses in this category."
        )
    else:
        highest = max(CATEGORIES, key=lambda c: expense_map.get(c, 0))
        highest_spend = expense_map.get(highest, 0)
        if highest_spend > 0:
            insight = (
                f"All categories are currently within their budget limits. "
                f"Your highest expenditure is in {highest} at ₹{highest_spend:.0f}."
            )
        else:
            insight = "All categories are well within their allocated budgets with zero expenses logged."
